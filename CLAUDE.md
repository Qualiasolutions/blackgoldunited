...
```typescript
// Don't use nested foreign keys - PostgREST has unreliable nested joins
...
- Database uses `snake_case` (e.g., `company_name`, `contact_person`)
- Always match exact database column names in queries
- Use `is_active` for soft deletes (NOT `deleted_at` or `deletedAt`)
