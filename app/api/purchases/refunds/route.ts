...
// GET /api/purchases/refunds - Get purchase refunds (debit notes marked as refunds)
export async function GET(request: NextRequest) {
  try {
    ...
    // Get debit notes that represent refunds
    let query = supabase
      .from('debit_notes')
      .select(`
        id,
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to fetch purchase refunds' }, { status: 500 })
    }

    // Format data for frontend
    const formattedData = data?.map((item: any) => ({
      id: item.id,
      refundNumber: item.debit_note_number,
...
// POST /api/purchases/refunds - Create purchase refund
export async function POST(request: NextRequest) {
  try {
    ...
    let refundNumber = 'PR-2025-0001'
    if (lastDebitNote?.debit_note_number) {
      const parts = lastDebitNote.debit_note_number.split('-')
      const lastNumber = parseInt(parts[parts.length - 1])
      refundNumber = `PR-2025-${String(lastNumber + 1).padStart(4, '0')}`
    }

    const { data, error } = await supabase
      .from('debit_notes')
      .insert({
        debit_note_number: refundNumber,
        supplier_id: body.supplierId,
        purchase_invoice_id: body.invoiceId,
        issue_date: body.issueDate || new Date().toISOString().split('T')[0],
        total_amount: body.amount,
        reason: body.reason || 'Purchase Refund',
    ...
    if (error) {
      console.error('Database error:', error)
      return NextResponse.json({ error: 'Failed to create purchase refund' }, { status: 500 })
    }
